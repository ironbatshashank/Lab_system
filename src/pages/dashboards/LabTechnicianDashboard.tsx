  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">Projects pending training verification</p>
      </div>

      <Card>
        <CardHeader>
          <h3 className="text-lg font-semibold">Pending Assessments ({projects.length})</h3>
        </CardHeader>
        <CardContent>
          {projects.length === 0 ? (
            <div className="text-center py-8">
              <Microscope className="w-12 h-12 text-gray-300 mx-auto mb-4" />
              <p className="text-gray-600">No projects pending training assessment</p>
            </div>
          ) : (
            <div className="space-y-3">
              {projects.map((project) => (
                <Link key={project.id} to={`/reviews/technician/${project.id}`}>
