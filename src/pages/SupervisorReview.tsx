...
export function SupervisorReview() {
  ...
  const handleApprove = async () => {
    ...
    setSubmitting(true);
    try {
      console.log('Starting approval process...', { approvalExists: !!approval, profileId: profile?.id, projectId: id });

      if (approval) {
        console.log('Updating existing approval...', approval.id);
      ...
      console.log('Updating project status to pending_hsm...');
      console.log('Current user ID:', profile?.id);
      console.log('Current user role:', profile?.role);
      console.log('Project ID:', id);

      // Debug: Check auth state from database perspective
      const { data: authDebug } = await supabase.rpc('debug_auth_info');
      console.log('Auth debug info from DB:', authDebug);

      const { data: updateData, error: projectError } = await supabase
        .from('projects')
        .update({ status: 'pending_hsm' })
        .eq('id', id)
        .select();

      console.log('Update result:', { data: updateData, error: projectError });
      ...
      showToast(`Failed to approve project: ${error.message || 'Unknown error'}`, 'error');
    } finally {
      setSubmitting(false);
    }
  };
